...
Planet.R_d(ps::AbstractEarthParameterSet) =
    gas_constant() / Planet.molmass_dryair(ps)
...
Planet.molmass_ratio(ps::AbstractEarthParameterSet) =
    Planet.molmass_dryair(ps) / Planet.molmass_water(ps)
Planet.R_v(ps::AbstractEarthParameterSet) =
    gas_constant() / Planet.molmass_water(ps)
...
Planet.LH_f0(ps::AbstractEarthParameterSet) =
    Planet.LH_s0(ps) - Planet.LH_v0(ps)
Planet.e_int_v0(ps::AbstractEarthParameterSet) =
    Planet.LH_v0(ps) - Planet.R_v(ps) * Planet.T_0(ps)
...
Planet.obliq_spline(ps::AbstractEarthParameterSet) = 0
Planet.lon_perihelion_spline(ps::AbstractEarthParameterSet) = 0
Planet.eccentricity_spline(ps::AbstractEarthParameterSet) = 0
