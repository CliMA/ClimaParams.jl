...
There are two key functions for parameter retrieval:
...
```@example howto
params = CP.get_parameter_values(toml_dict, ["gas_constant", "gravitational_acceleration"])
params.gas_constant
...
Name maps allow you to map global parameter names to local variable names for convenience. This is especially useful when you want shorter, more intuitive names in your code.
...
Here's how [`Thermodynamics.jl`](https://github.com/CliMA/Thermodynamics.jl) uses ClimaParams in practice:
